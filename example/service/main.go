...
func main() {
	...
	/*
		group  cluster:ip
		DEFAULT DEFAULT:10 cluster a:11 cluster b:12:alive
		group a cluster b:13:alive
		group b cluster b:14
	*/
	...
	//group=DEFAULT 增加 cluster-b:112
	ExampleServiceClient_RegisterServiceInstance(client, vo.RegisterInstanceParam{
		Ip:          "10.0.0.112",
		Port:        8848,
		ServiceName: "demo.go",
	...
	//wait for client pull change from server
	fmt.Println("--------------DeRegisterServiceInstance----222")
	time.Sleep(2 * time.Second)
